...
export const fetchDepartments = async () => {
  let departments;
  try {
    const response = await axios.get("https://employee-management-system-api-lyart.vercel.app/api/department/", {
      headers: {
...
export const getEmployees = async (id) => {
  ...
  try {
    const response = await axios.get(
      `https://employee-management-system-api-lyart.vercel.app/api/employee/department/${id}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
